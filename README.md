...
- **支持多物料**：一条需求可以包含多种物料（如"采购 50 箱 一次性无菌注射器和 20 盒医用外科口罩"），逐项比价、逐行下单，审批按**合计金额**判定。
- **缺信息会停下来问，并且能接着跑**：需求缺物料或数量时任务进入等待澄清状态，用户在页面补充后从解析阶段续跑（不是重建任务）；物料名称支持主数据别名，例如"无菌注射器"可匹配到 `一次性无菌注射器`。
...
tests/             258 项测试（含结构约束与载荷往返等防回归用例）
...
2. 执行页观察：展开左栏「工程细节」折叠区，技能标签从「仅描述」逐个变为「已加载全文」；中间任务树展开三类子 Agent 与其工具调用；右侧时间线默认只显示关键事件（阶段推进、委派、审批、重试与错误），点「全部事件」可看工具调用与上下文压缩等细节。
3. 打开数据台，开启「让供应商 B 资质过期」，回到看板重新提交，观察系统淘汰该供应商并在比价结论中说明改选理由。
4. 提交 `采购 3000 箱 一次性无菌注射器`，金额超阈值触发审批卡片，现场批准或驳回（驳回会回退到比价阶段重跑）。
5. 打开评测页，点「运行评测」，查看指标与失败用例。

详细分镜与讲解要点见 [docs/demo-script.md](docs/demo-script.md)。

**要自己动手操作，看 [docs/operation-guide.md](docs/operation-guide.md)**：启动停止、四个页面能做什么、八个演示场景的具体操作、出错排查、面试追问速查。
...
| 指标 | 离线确定性模式 | 真实模型模式（实测于 2026-09-23，演示数据扩充前） |
| --- | --- | --- |
| 用例总数 | 30 | 30 |
| 成功率 | 100%（30/30） | 100%（30/30） |
| 人工介入率 | 26.7% | 26.7% |
| 平均单任务耗时 | 约 0.8–0.9 s | 约 28.5 s |
| 框架委派 / 降级次数 | 90 / 0 | 90 / 0 |
| 每任务 token 总量 | 0（离线模型无用量，不造假） | 均值 57,377，最大 62,553 |
| 单次调用 token 峰值 | 由摘要中间件估算 | 均值 4,056，最大 4,401 |
| 上下文压缩率 | 87.1%（4676 → 604 token，12 段工具返回基准） | — |
| 自动化测试 | 258 项全部通过 | — |
| 静态检查 | `ruff check .` 零告警 | — |

另有一组高难度用例（模糊表述、物料别名、边界数量、多物料、超经营范围、供应商不足等）：
...
真实模型实测（2026-09-24）：**13/13 通过**、人工介入率 30.8%、平均单任务 21.5 秒、
每任务 token 均值 50,316（最大 64,322）、框架委派 33 次零降级。
离线确定性模式下同样 13/13（`pytest tests/test_hard_cases.py` 每次都会跑一遍）。
这组用例不是为了刷高成功率，而是为了证明系统在模糊输入下会**停下来提问**而不是猜。

离线模式下这也是一道回归门：`tests/test_hard_cases.py` 会把整组用例跑一遍，
用例集的前提（例如"只剩一家可用供应商"）一旦被演示数据变化破坏，测试会直接报出来。

## 推送到远程仓库

仓库目前只在本地。要备份或分享，先在本机安装 [GitHub CLI](https://cli.github.com/)（或手动在网页上建一个**空**仓库），然后：

```bash
gh auth login                                   # 首次使用需要授权
gh repo create procurement-agent --private --source=. --remote=origin --push
```
