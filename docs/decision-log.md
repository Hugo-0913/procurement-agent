...
新增第二种物料 `订书机`（SKU `ST-STAPLER-24`，别名 `订书器`、`stapler`），三家供应商各有报价与历史成交价。
老库通过幂等分支 `_ensure_stapler` 补齐，保证已有演示库也能演示多物料。

### 真机验证

```
采购 50 箱 A4 纸和 20 个订书机，成本中心 CC-1001
  → state=COMPLETED  物料=A4 纸×50 + 订书机×20  订单行数=2  委派=framework×3

采购 2000 箱 A4 纸和 1000 个订书机，成本中心 CC-1001
