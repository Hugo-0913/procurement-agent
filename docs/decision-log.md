...
值得记录的是：H-01（"帮我买点办公用纸"）与 H-02（"采购 10 个鼠标"）在真实模型下都正确停在
解析阶段向用户提问，没有猜数量、也没有硬套 A4 纸——这正是把"缺字段必须澄清"写成确定性分支
而不是依赖模型自觉的价值。

### 仍未解决的缺口（如实记录）

**交期可行性未校验。** 用例 H-06（"紧急采购 3000 箱 A4 纸，明天必须到货"）之所以转人工，
...
用例 H-06（"紧急采购 3000 箱 A4 纸，明天必须到货"）现在的表现：

```
命中规则 = ['订单金额 ¥59520.00 > 阈值 ¥50000.00', '交期无法满足期望到货日期，需人工确认']
比价结论 = 推荐 恒信纸业，综合成本最低 ¥59520.00；
